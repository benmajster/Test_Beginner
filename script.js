...
// Theme: use saved preference or system default on first run
let theme = loadTheme(); // "light" | "dark"
applyTheme(theme);

// ---------- Startup ----------
allowNegativesEl.checked = allowNegatives;
sortEl.value = sortMode;
if (darkModeEl) darkModeEl.checked = theme === "dark";
updateSummary();
updateClearAllState();
render();
...
function getVisibleCounters() {
  ...
  switch (sortMode) {
    ...
    default: list.sort(byCreatedDesc);
  }
  return list;
}
...
function applyTheme(value) {
  // We set an attribute on <html> so CSS variables can switch
